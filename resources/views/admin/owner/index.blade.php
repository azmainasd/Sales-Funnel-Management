        ...
        <div class="col-12">
            <div class="card">
              ...
              <div class="card-body">
                ...
                <a class="btn btn-outline-success custom-btn float-left" href="{{route('owner.create')}}"><i class="fas fa-plus"></i> Add New</a>
                @endcan
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      @can('Master-Data-Edit',Auth::user())
                      <td>
                        <a class="mr-3" href="{{ route('owner.edit',[$owner->id]) }}"><i class="fas fa-edit"></i></a>
                      </td>
                      @endcan
                      @can('Master-Data-Delete',Auth::user())
                      <td>
                        <form id="delete-form-{{ $owner->id }}" method="post" action="{{ route('owner.destroy',$owner->id) }}" style="display: none">
                          {{ csrf_field() }}
                          {{ method_field('DELETE') }}
                        </form>
                        <a class="text-danger" href="" onclick="
                            if(confirm('Are you sure, You Want to delete this ? If you delete this owner all your related data on sales funnel will be destroyed!'))
                            {
                              event.preventDefault();
                              document.getElementById('delete-form-{{ $owner->id }}').submit();
                            }
                            else{
                              event.preventDefault();
